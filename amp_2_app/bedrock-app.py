...
example_instruction = "以下の<text>から何号機か特定してください。説明はいらないので何号機かだけを返してください。何号機かは<list>のいずれかです。"
...
def prompt_construction(modelId, instruction="[instruction]", prompt="[input_text]", tiers="[tier_list]"):
...
def summarize(modelId, input_text, instruction_text, tier_list, max_tokens, temperature, top_p):
  # Initializing the bedrock client using AWS credentials
  boto3_bedrock = bedrock.get_bedrock_client(
      region=os.environ.get("AWS_DEFAULT_REGION", None))
  
  full_prompt = prompt_construction(modelId, instruction_text, input_text, tier_list)
...
with gr.Blocks() as demo:
  with gr.Row():
    gr.Markdown("# OCR抽出情報に対する設備階層候補リストのマッチング")
  ...
  with gr.Row():
    with gr.Column(scale=4):
      ...
      input_text = gr.Textbox(label="OCR抽出情報 (<text>タグ)", placeholder="クレンジング対象のテキストを入力")
      tier_list = gr.Textbox(label="候補設備階層リスト (<tiers>タグ)", placeholder="""{"共用", "１号", "２号"}""")
  ...
  summarize_btn.click(fn=summarize, inputs=[modelId, input_text, custom_instruction, tier_list, max_new_tokens, temperature, top_p], outputs=output, 
                            api_name="summarize")
